...
import numpy as np
from bigImage_processing import *
...
def Evaluate(sess):
    ...
    for it in range(test_iteration):
        ...
        test_batch_x = data_augmentation(batch_x)
        test_batch_x = color_preprocessing(batch_x)
...
class SE_Inception_resnet_v2():
    ...
    #def image_big2small(self, x, scope):
        #with tf.name_scope(scope):
            #x = conv_layer(x, filter=32, kernel=[9,9], stride=5, layer_name=scope+'1')
            #x = conv_layer(x, filter=32, kernel=[5,5], stride=2, layer_name=scope+'2')
            #x = conv_layer(x, filter=64, kernel=[5,5], layer_name=scope+'3')
            #x = conv_layer(x, filter=64, kernel=[3,3], stride=2, layer_name=scope+'4')
            #x = conv_layer(x, filter=64, kernel=[3,3], layer_name=scope+'5')
        #return x

    def Build_SEnet(self, input_x):

        #input_x = self.image_big2small(input_x, scope='B2S')
        # size 32 -> 96
...
train_x, train_y, test_x, test_y = get_file('./train/')
